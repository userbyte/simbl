...
function SETUP_xLogin() {
    ...
    .then(post => {
        ...
        if (responsestatus === 'success') {
            ...
            s.innerHTML = "<b>login error:<br>"+"an unknown error has occurred :("+"</b>";
            s.setAttribute('class', 'status status-error');
            return;
        }
    })
...
function SETUP_doSetup(xapiURL, simblapiURL, xapi_token) {
    ...
    if (xapi_token === "" || xapi_token === null) {
        s.innerHTML = "<b>setup error:<br> token is missing! this shouldnt be possible... wtf?</b>";
        s.setAttribute('class', 'status status-error');
        return;
    }
    ...
    .then(post => {
        ...
        if (responsestatus === 'success') {
            s.innerHTML = "<b>setup success<br>redirecting back to the homepage...</b>";
            s.setAttribute('class', 'status status-success');
            setTimeout(function() {
                //window.history.back();
                location.href = ''; return false; // reload previous page for updated data
            }, 700);
        } else if (responsestatus === 'failed') {
            err = post.error;
            ...
            s.innerHTML = "<b>setup error:<br>"+"an unknown error has occurred :("+"</b>";
            s.setAttribute('class', 'status status-error');
            return;
        }
    });
